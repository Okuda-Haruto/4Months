...
ConstantBuffer<Fog> gFog : register(b5);
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    float2 backGroundUV = input.position.xy / gFog.windowSize;
    float4 backGroundColor = gBackGround.Sample(gSampler, backGroundUV);
    
    float3 fogColor = saturate(backGroundColor.rgb + 0.1f);
    ...
    {
        t = fog / 0.75f;
        output.color.rgb = lerp(backGroundColor.rgb, fogColor, t);
    }
