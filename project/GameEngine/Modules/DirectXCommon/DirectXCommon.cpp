...
//描画後処理
void DirectXCommon::RenderPostDraw(std::string textureName) {
...
//RootSignature作成
ComPtr <ID3D12RootSignature> DirectXCommon::VoxelRootSignatureInitialvalue() {
	D3D12_DESCRIPTOR_RANGE descriptorRange[1] = {};
	descriptorRange[0].BaseShaderRegister = 0;	//0から始める
	descriptorRange[0].NumDescriptors = 1;	//数は1つ
	descriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;	//SRVを使う
	descriptorRange[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;	//Offsetを自動計算
	D3D12_DESCRIPTOR_RANGE backGroundDescriptorRange[1] = {};
	backGroundDescriptorRange[0].BaseShaderRegister = 1;	//1から始める
	backGroundDescriptorRange[0].NumDescriptors = 1;	//数は1つ
	backGroundDescriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;	//SRVを使う
	backGroundDescriptorRange[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;	//Offsetを自動計算
	...
	rootParameters[9].DescriptorTable.pDescriptorRanges = backGroundDescriptorRange;	//Tableの中身の配列を指定
	rootParameters[9].DescriptorTable.NumDescriptorRanges = _countof(backGroundDescriptorRange);	//Tableで利用する数
	descriptionRootSignature.pParameters = rootParameters;	//ルートパラメータ配列へのポインタ
	descriptionRootSignature.NumParameters = _countof(rootParameters);	//配列の長さ
...
//RootSignature作成
ComPtr <ID3D12RootSignature> DirectXCommon::InstancingVoxelRootSignatureInitialvalue() {
	D3D12_DESCRIPTOR_RANGE descriptorRange[1] = {};
	descriptorRange[0].BaseShaderRegister = 0;	//0から始める
	descriptorRange[0].NumDescriptors = 1;	//数は1つ
	descriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;	//SRVを使う
	...
	rootParameters[7].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;	//CBVを使う
	rootParameters[7].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;	//VertexShaderで使う
	rootParameters[7].Descriptor.ShaderRegister = 1;	//レジスタ番号1を使う
	rootParameters[8].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;	//CBVを使う
	rootParameters[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;	//PixelShaderで使う
	rootParameters[8].Descriptor.ShaderRegister = 5;	//レジスタ番号5を使う
	rootParameters[9].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;	//DescriptorTableを使う
...
Microsoft::WRL::ComPtr<ID3D12RootSignature> DirectXCommon::GlowRootSignatureInitialvalue() {
